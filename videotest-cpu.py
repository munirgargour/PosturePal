...
def capturephoto(): # used for capturing baseline photos
    ...
    while baseline_frame is None:
        ...
        if not ret:
            ...
            cv2.putText(frame, instruction, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 0, 0), 2)
...
def analyzebaseline():
    ...
    if len(instances) > 0:
        best_idx = instances.scores.argmax().item()  #updated this func and posturecheck to label whomever is detected with best confidence as "object 0" 
        best_detection = {
            'bbox': instances.pred_boxes[best_idx].tensor.numpy()[0],
            'score': instances.scores[best_idx].item(),
            'keypoints': instances.pred_keypoints[best_idx].numpy() if hasattr(instances, "pred_keypoints") else None,
            'id': 0  # Always label as person 0
			}
...
def posturecheck(keypointlist):
    while True:
        ...
        if current_nose is not None:
            print("Current nose Y",current_nose[1], "Baseline nose Y:" , keypointlist[0][1])
